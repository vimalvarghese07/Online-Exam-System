...
import { acceptOrdeny, updateQSetter } from "../../helper/helper";
import { QsetterValidate } from "../../helper/validate";
import useFetch from "../../hooks/fetch.hook";
import CancelIcon from '@mui/icons-material/Cancel';
import "../style.scss";
const username = localStorage.getItem("username");

export default function QSetter(props) {

    const navigate = useNavigate();
    const user = useFetch(`getUser/${username}/QSetter`);
    const adminReq = useFetch(`getExamReq/qsetter/${username}`);
    const bottomReq = useFetch(`getAcceptedExamRequest/qsetter/${username}`)
    ...
    const formik = useFormik({
        ...
        validate:QsetterValidate,
        enableReinitialize: true,
        validateOnBlur: false,
        validateOnChange: false,
        onSubmit: async values => {
            values = await Object.assign(values, { profile: file || "" });
            const response = updateQSetter(values, "profile");
    ...
    const handleClickExam = (examList, admin) => {
        console.log(`${examList},${admin}`);
        navigate(`/setQuestion/${examList}/${admin}`);
    }
                        ...
                        <div className="body">
                            <form onSubmit={formik.handleSubmit}>
                                ...
                                <div className="fields">
                                    ...
                                    {apiData2?.fname ? <span><button type="button"  onClick={() => getProfileInfo()}> View Profile </button></span> : <button type="submit">Update</button>}
                                </div>
                            </form>
                        </div>
                        ...
                        <div className="insideR">
                            {apiData?.map((item, index) => (
                                <div key={index} className="contentBox">
                                    <span><b className="adminName">{item.admin}</b> has assigned you as the Question Paper Setter for the Exam <b className="listExam">{item.examList}</b> </span>
                                    <div className="btns">
